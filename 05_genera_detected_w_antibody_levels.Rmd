...
Extract genus name order based on highest mean proportion and use this as the order for the plots
...
```{r, echo = FALSE, eval = FALSE, fig.height=9}
endia_prop_samples_per_genus_plot + endia_mean_rpk_plot + plot_annotation(tag_levels = "A")
```
...
**Supplementary Fig. 2:** Detection of viral antigens corresponding to 72 genera in the ENDIA cohort, stratified by case (n = 44) and control (n = 99) groups. (A) Proportion of samples in which antigens were detected. Numbers adjacent to each bar indicate the number of samples contributing to the respective proportion. (B) $\log_{10}$-transformed mean normalised read counts, expressed as reads per thousand (RPK), for each genus in case and control groups.

```{r, echo = FALSE}
ggsave("figures/endia_combined_rpk_and_samples_genera_plot.png", endia_combined_rpk_and_samples_genera_plot, width = 15, height = 10)
ggsave("figures/endia_combined_rpk_and_samples_genera_plot.svg", endia_combined_rpk_and_samples_genera_plot, width = 15, height = 10, dpi = 320)
```
...
vigr_prop_samples_per_genus_plot <- vigr_prop_samples_per_genus %>% 
  mutate(condition = factor(Condition, levels = c("Control", "Case"))) %>% # change order so cases come above controls in the plot
  ggplot(aes(x = taxon_genus, y = proportion, fill = condition)) +
  geom_col(position = "dodge2") +
  geom_text(aes(label = round(no_pos_samples)), 
  ...
  guides(fill = guide_legend(reverse = TRUE)) +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.05)),  # adds 1% space on the right of the plot
    breaks = c(0, 0.25, 0.5, 0.75, 2.0),
...
```{r, echo=FALSE, fig.height=8}
vigr_mean_rpk_plot <- vigr_mean_rpk %>% 
  mutate(condition = factor(Condition, levels = c("Control", "Case"))) %>% # change order so cases come above controls in the plot
  ggplot(aes(x = taxon_genus, y = log10(mean_abundance + 1), fill = condition)) +
  geom_col(position = "dodge2") +
  coord_flip() +
...
```{r, echo=FALSE, eval = FALSE, fig.height=9, fig.width=11}
vigr_prop_samples_per_genus_plot + vigr_mean_rpk_plot
```
...
**Supplementary Fig. 3:** Detection of viral antigens corresponding to 74 genera in the VIGR cohort, stratified by case (n = 21) and control (n = 21) groups. (A) Proportion of samples in which antigens were detected. Numbers adjacent to each bar indicate the number of samples contributing to the respective proportion. (B) $\log_{10}$-transformed mean normalised read counts, expressed as reads per thousand (RPK), for each genus in case and control groups.

```{r, echo = FALSE}
ggsave("figures/vigr_combined_rpk_and_samples_genera_plot.png", vigr_combined_rpk_and_samples_genera_plot, width = 15, height = 12)
ggsave("figures/vigr_combined_rpk_and_samples_genera_plot.svg", vigr_combined_rpk_and_samples_genera_plot, width = 15, height = 12, dpi = 320)
```
