...
Use mixed model with poisson distribution for paper on both cohorts using the `glmmmTMB` R package
         
```{r}
library(glmmTMB)

vigr_ev_data <- vigr_top_genera_virscan_log_rpk %>% filter(taxon_genus == "Enterovirus")


vigr_ev_model_poisson <- glmmTMB(case ~ log_genus_rpk + HLA_Status + Age + Sex + (1|Nest) + (1|My_name),
                                family = poisson(link = "log"),
                                data = vigr_ev_data)
...
endia_ev_data <- endia_all_top_genera_onset_log_rpk %>% filter(taxon_genus == "Enterovirus")


endia_ev_model_poisson <- glmmTMB(formula = case ~ log_genus_rpk + infant_HLA + age_sample_collection_month + infant_sex + (1|deidentified_nest_id_new) + (1|mother_id),
       family = poisson(link = "log"),
        weights = weightTEDDY,
       data = endia_ev_data)
...
```{r}
library(gtsummary)
endia_ev_model_poisson %>% tbl_regression(exponentiate = TRUE)
```
...
```{r}
gtsave(tidy_models_data_table, "figures/glm_results_onset_ev.png")
```
